...
#include "CFDSolver/Solver.hpp"

void Interface::SetData(Solver& solver)
{
...
void Interface::HandlePannel()
{
    ...
    if (in_model_creator_mode)
    {
        ...
        // Size Options
        float label_offset_y = 25.0f;
        x_rec_size = (width - 2 * x_offset_outer - 5 * x_offset_inner) / 4;
        local_size = x1 + x_offset_outer + x_offset_inner;
        for (int i = 0; i < 4; i++)
        {
            float box_width = (i >= 2) ? x_rec_size * 1.25f : x_rec_size * 0.75f;

            int text_width = MeasureText(size_lables[i], 20);
            DrawText(size_lables[i], local_size + (box_width - text_width) / 2, current_y - label_offset_y, 20, BLACK);

            if (GuiTextBox(Rectangle(local_size, current_y, box_width, size), size_inputs[i], 32, size_edit_mode[i]))
            {
                size_edit_mode[i] = !size_edit_mode[i];
                if (size_edit_mode[i]) strcpy(size_inputs[i], "");
                else
                {
                    if (i < 2)
                    {
                        if (IsValidInt(size_inputs[i], false))
                            (i == 0 ? nx : ny) = std::max(3, std::stoi(size_inputs[i]));
                        std::sprintf(size_inputs[i], "%d", (i == 0 ? nx : ny));

                        SanatiseFaceSets();
                    }
                    else
                    {
                        if (IsValidFloat(size_inputs[i], false))
                            (i == 2 ? dx : dy) = std::max(0.0001f, std::stof(size_inputs[i]));
                        std::sprintf(size_inputs[i], "%.2e", (i == 2 ? dx : dy));
                    }
                }
            }
            else if (!size_edit_mode[i] && i < 2)
            {
                std::sprintf(size_inputs[i], "%d", (i == 0) ? nx : ny);
            }

            local_size += x_offset_inner + box_width;
        }
        current_y += y_offset_outer * 2;

        if (GuiButton(Rectangle(x1 + (width - x_rec_size * 2) / 2, current_y, x_rec_size * 2, size), "Reset View")) ResetView();
    }
    else if (in_render_mode)
    {
        ...
        DrawText(stremline_options[0], x1 + x_offset_block + x_rec_size + text_offset, current_y + y_offset_inner * 1 + size * 0 + size / 2 - text_size / 2, text_size, BLACK);
        if (GuiTextBox(Rectangle(x1 + x_offset_block, current_y + y_offset_inner * 2 + size * 1, x_rec_size, size), stremline_option_inputs[1], 32, stremline_option_editmode[1]))
        {
            ...
            else if (!IsValidInt(stremline_option_inputs[1], true)) std::sprintf(stremline_option_inputs[1], "%d", streamline_steps); 
            else
            {
                int new_streamline_steps = std::stoi(stremline_option_inputs[1]);
                if (streamline_steps != new_streamline_steps)
                {
                    streamline_steps = new_streamline_steps;
                    GenerateStreamlines();
                }
            }
        }
        DrawText(stremline_options[1], x1 + x_offset_block + x_rec_size + text_offset, current_y + y_offset_inner * 2 + size * 1 + size / 2 - text_size / 2, text_size, BLACK);
        ...
        bool previous_is_reverse = reverse_streamlines;
        DrawText(stremline_options[2], x1 + x_offset_block + x_rec_size + text_offset, current_y + y_offset_inner * 3 + size * 2 + size / 2 - text_size / 2, text_size, BLACK);
        ...
        if (previous_is_reverse != reverse_streamlines) { GenerateStreamlines(); }
        current_y += local_size + y_offset_outer;
        ...
        if (GuiTextBox(Rectangle(x1 + x_offset_block, current_y + y_offset_inner * 1 + size * 0, x_rec_size, size), v_arrow_inputs, 32, v_arrow_editmode))
        {
            ...
            else std::sprintf(v_arrow_inputs, "%.2f", v_arrow_scale);
        }
        else if (!v_arrow_editmode) { std::sprintf(v_arrow_inputs, "%.2f", v_arrow_scale); } // the value could have been changes using shortcuts
        DrawText("scale", x1 + x_offset_block + x_rec_size + text_offset, current_y + y_offset_inner * 1 + size * 0 + size / 2 - text_size / 2, text_size, BLACK);
        current_y += local_size + y_offset_outer;
    ...
    if (GuiButton(Rectangle((width - xsize) / 2 + x1, y2 - yoffset - ysize, xsize, ysize), in_model_creator_mode ? "Start" : (is_solving ? "Stop" : "Reset")))
    {
...
void Interface::HandleInput()
{
    ...
    else
    {
        // Scale Velocity
        if (IsKeyDown(KEY_UP))
            v_arrow_scale *= 1.0f + zoom_sensitivity;
        if (IsKeyDown(KEY_DOWN))
            v_arrow_scale *= 1.0f / (1.0f + zoom_sensitivity);
    }
}